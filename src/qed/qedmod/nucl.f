...
!
! Originally from: f/potgen/nucl.f
!
c       =================================================
        subroutine SHAB_nucl(maxii,r,unuc, set_fermi_params)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       set_fermi_params ... if set to .false., the aa, tt and cc parameters
c                    have to specified by hand.
c
c       Output parameters:
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        common /SHAB_fermi/aa,tt,cc,ro0,rnucl0
        real*8 r(maxii),unuc(maxii)
        logical set_fermi_params
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
25        format
     &    ( 2x,'Rnucl     =',e14.6,1x,'[a.u.] =',f8.4,1x,'[Fermi]',
     &     /2x,'Sqrt<R^2> =',e14.6,1x,'[a.u.] =',f8.4,1x,'[Fermi]')
c
...
45      format (2x,'Parameter c=',e13.6,1x,'[a.u.] =',f8.4,
     &  1x,'[Fermi]',/2x,'Parameter t=',e13.6,1x,
     &  '[a.u.] =',f8.4,1x,'[Fermi]')
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        dnorm=cc**3*(1+(pi*aa/cc)**2-6*(aa/cc)**3*SHAB_sk(3,-cc/aa))/3
        ro0=z/dnorm
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
c       SHAB_RM(n)=<ro(r)*r^n>/ro0;  ro0=2*ro(c)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
	rm1=SHAB_rm(1,cc,aa)
        vnuc(2)=-rm1*ro0
        vnuc(4)=B0/6.d0
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
c       V(r)=-Z*SHAB_erf(dsqrt(1.5)*r/Rms)/r
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        write( *,65)
        write(11,65)
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        do i=1,ii
          if (knucl.eq.1) vi=SHAB_vpot_fermi(z,cc,aa,r(i))
          if (knucl.eq.3) vi=SHAB_vpot_gauss(z,rnucl0,r(i))
          unuc(i)=z+vi
        enddo
        inucl=ii
        do i=ii,1,-1
...
c       =================================================
        real*8 function SHAB_ro_nucl(r)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        implicit real*8(a-h,o-z)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        common /SHAB_const/cl,z
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        if (knucl.eq.3) then
          SHAB_ro_nucl=z*4*pi*(1.5d0/(pi*rnucl0**2))**1.5d0*
...
c       =================================================
        function SHAB_vpot_fermi(z,c,a,r)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        parameter (pi=3.1415926535897932385d0)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        dn=1+(pi*a/c)**2-6d0*((a/c)**3)*SHAB_SK(3,-c/a)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        if (r.lt.c) then
        B1=-6d0*((a/c)**3)*SHAB_SK(3,-c/a)+6d0*((a/c)**3)*
     &     SHAB_SK(3,(r-c)/a)
        B2=r/c*(3d0/2d0+((pi*a/c)**2)/2d0-3d0*((a/c)**2)*
     &     SHAB_SK(2,(r-c)/a))
        B3=-((r/c)**3)/2d0
        V=Z/dn*(B1+B2+B3)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        B1=dn
        B2=3d0*((a/c)**2)*(r/c*SHAB_SK(2,(c-r)/a)+2d0*a/c*
     &     SHAB_SK(3,(c-r)/a))
        V=Z/dn*(B1+B2)
        endif
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       =================================================
        function SHAB_vpot_gauss(z,rms,r)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       external SHAB_erf
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        SHAB_vpot_gauss=-z*SHAB_erf(dsqrt(1.5d0)*r/rms)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
	return
        end
c       =================================================
        function SHAB_sk(k,x)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        if (m.eq.1) then
          SHAB_rm=c*c/2+a**2*SHAB_sk(2,-c/a)+2*a**2*(pi**2/12)
        return
        endif
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        SHAB_rM=c**(M+1)/(M+1)
...
10      N=N+1
        dn=N
        add=((-1)**N)*(SHAB_fxneax(M,dN/A,C)-DEXP(-dN*C/A)*
     &  SHAB_fxneax(M,dN/A,0d0))
        SHAB_rm=SHAB_rm+add
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
30      N=N+1
        dN=N
        add=((-1)**(N+1))*(-SHAB_fxneax(M,-dN/A,C))
        IF(N.eq.1) RM1=add
        SHAB_RM=SHAB_RM+add
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       =================================================
        FUNCTION SHAB_fxneax(N,A,X)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        implicit real*8 (A-H,O-Z)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        SUM=0d0
        DO 10 l=0,N
c       SUM=SUM+SHAB_dfac(n)/SHAB_dfac(n-l)*((A*X)**(n-l))*(-1)**l
        SUM=SUM+SHAB_dfac(n)/SHAB_dfac(n-l)*(SHAB_stpdi(A*X,n-l))*
     &      (-1)**l
10      CONTINUE
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        SHAB_fxneax=SUM/(A**(N+1))
c       - - - - - - - - - - - - - - - - - - - - - - - - -
        RETURN
        END
c       =================================================
        function SHAB_dfac(n)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
1000    return
        END
c       =================================================
        DOUBLE PRECISION FUNCTION SHAB_erf(x)
c       - - - - - - - - - - - - - - - - - - - - - - - - -
...
c       - - - - - - - - - - - - - - - - - - - - - - - - -
   10   IF (ax.GT.4.0D0) GO TO 20
     ...
     +        q(6))*ax+q(7))*ax + q(8)
        SHAB_erf = 0.5D0 + (0.5D0-exp(-x*x)*top/bot)
        IF (x.LT.0.0D0) SHAB_erf = -SHAB_erf
        RETURN
c       - - - - - - - - - - - - - - - - - - - - - - - - -
   20   IF (ax.GE.5.8D0) GO TO 30
        ...
        SHAB_erf = (c-top/ (x2*bot))/ax
        SHAB_erf = 0.5D0 + (0.5D0-exp(-x2)*SHAB_erf)
        IF (x.LT.0.0D0) SHAB_erf = -SHAB_erf
        RETURN
c       - - - - - - - - - - - - - - - - - - - - - - - - -
   30   SHAB_erf = sign(1.0D0,x)
        RETURN
        END
